...
# Resolve target terminal to send escape sequence. Copy bindings provide the
# initiating client's TTY explicitly; SSH_TTY may be stale after reattaching or
# may belong to another client attached to the same session.
pane_active_tty=$(tmux list-panes -F "#{pane_active} #{pane_tty}" | awk '$1=="1" { print $2 }')
target_tty="${TMUX_COPY_TTY:-${SSH_TTY:-$pane_active_tty}}"
